...
The project currently behaves like a lightweight demo web server. `src/main.cpp` registers a few example routes, enables static file serving from `root/`, initializes logging, creates the MySQL pool, starts worker threads, and then runs the epoll event loop.
...
|   |-- config/               # Runtime configuration implementation
|   |-- webserver/            # Main server lifecycle and epoll loop
|   |-- http/                 # HTTP parsing, response, routing, and JSON code
|   |-- log/                  # Logger implementation
|   |-- timer/                # Timer implementation
|   `-- cgi_mysql/            # MySQL connection pool implementation
...
|-- tests/                    # Manual smoke tests and experimental test programs
|-- scripts/                  # Helper scripts
|-- docker/                   # Container support files, including MySQL init SQL
|-- build/                    # Local build output, generated by make
|-- Dockerfile
|-- docker-compose.yml
`-- Makefile
```
...
- Python 3 with `requests`, only if you want to run `tests/http_smoke.py`
...
The build creates a `build/server` executable.
...
```bash
./build/server -p 8080 -t 16 -s 8 -l 1 -m 3
```
...
The sample routes are registered in `src/main.cpp`:
